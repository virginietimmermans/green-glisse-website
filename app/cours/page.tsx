      ...
      <section className="py-12 bg-[#F4E9CD]">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            {[
              { icon: "🤿", label: "Combinaison" },
              { icon: "🏄", label: "Planche de surf" },
              { icon: "🛡️", label: "Assurance" },
              { icon: "🩺", label: "Secouriste sur place" },
            ].map((item) => (
              <div key={item.label} className="text-center">
                <div className="text-4xl mb-2">{item.icon}</div>
                <p className="text-sm font-semibold text-[#1C1C1C]" style={{ fontFamily: "var(--font-montserrat), sans-serif" }}>
                  {item.label}
                </p>
              </div>
            ))}
